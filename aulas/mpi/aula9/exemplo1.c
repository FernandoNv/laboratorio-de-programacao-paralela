...
int main(int argc, char *argv[]){ /* mpi_bsend.c  */
    ...
    if (meu_ranque == 0) printf("Alocados %d bytes \n", tam_buffer);
    ...
    for (i = 1; i <= (num_procs / 2); i += i){
        ...
        MPI_Bsend(&reducao, tam_buffer, MPI_BYTE, destino, etiq, MPI_COMM_WORLD);
        // MPI_Bsend(&reducao, cont, MPI_INT, destino, etiq, MPI_COMM_WORLD);
        MPI_Recv(&recebido, tam_buffer, MPI_BYTE, destino, etiq, MPI_COMM_WORLD, &estado);
